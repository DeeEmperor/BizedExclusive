// Static outfit catalogue.
// This is the single source of truth for all outfits shown in the Gallery and Product pages.
// To add a new outfit, append an entry to this array and redeploy.

export interface StaticOutfit {
  id: string;
  name: string;
  slug: string;
  description: string;
  image: string;
  price: number | null;
  currency: string;
}

export const OUTFITS: StaticOutfit[] = [
  {
    id: "outfit-001",
    name: "Elegant vintage shirt",
    slug: "elegant-vintage-shirt",
    description: "A well tailored vintage shirt that exudes sophistication and style.",
    image: "/assets/IMG_0045.jpg",
    price: 1500000,
    currency: "NGN",
  },
  {
    id: "outfit-002",
    name: "Classic Attire",
    slug: "classic-attire",
    description: "A classic style for the modern and classy gentleman.",
    image: "/assets/IMG_0050.jpg",
    price: 2000000,
    currency: "NGN",
  },
  {
    id: "outfit-003",
    name: "Embroidered Two Piece",
    slug: "embroidered-two-piece",
    description: "Sharp business and embroidered attire designed for the gentle man who values both style and comfort at any location.",
    image: "/assets/IMG_1182.jpg",
    price: 2500000,
    currency: "NGN",
  },
  {
    id: "outfit-004",
    name: "Distinguished Agbada",
    slug: "distinguished-agbada",
    description: "A distinguished ensemble that makes a powerful statement at any traditional gathering.",
    image: "/assets/IMG_1191.jpg",
    price: 4500000,
    currency: "NGN",
  },
  {
    id: "outfit-005",
    name: "2 Colored Fìlà Gọ̀bì",
    slug: "2-colored-fila-gobi",
    description: "Your stylish and captivating trad cap for the outstanding gentleman.",
    image: "/assets/IMG_2331.jpg",
    price: 350000,
    currency: "NGN",
  },
  {
    id: "outfit-006",
    name: "Executive Collection",
    slug: "executive-collection",
    description: "Premium executive wear designed for leaders who command attention with their impeccable style.",
    image: "/assets/IMG_3257.jpg",
    price: 3000000,
    currency: "NGN",
  },
  {
    id: "outfit-007",
    name: "Stylish Embroidery",
    slug: "stylish-embroidery",
    description: "A perfect blend of style and professionalism with embroidered attire.",
    image: "/assets/IMG_4119.jpg",
    price: 2800000,
    currency: "NGN",
  },
  {
    id: "outfit-008",
    name: "Classic Business Attire",
    slug: "classic-business-attire",
    description: "Classy business wear that never goes out of style, perfect for making a lasting impression.",
    image: "/assets/IMG_4457.jpg",
    price: 2200000,
    currency: "NGN",
  },
  {
    id: "outfit-009",
    name: "Multi-colored Fìlà",
    slug: "multi-colored-fila",
    description: "High-end cap that showcases exceptional craftsmanship and attention to detail.",
    image: "/assets/IMG_1023.JPEG",
    price: 400000,
    currency: "NGN",
  },
  {
    id: "outfit-010",
    name: "Casual Attire",
    slug: "casual-attire",
    description: "Sharp and casual look that steals the show at any outdoor event.",
    image: "/assets/IMG_1257.JPG",
    price: 1800000,
    currency: "NGN",
  },
  {
    id: "outfit-011",
    name: "Executive Presence",
    slug: "executive-presence",
    description: "Command attention with this executive look that blends authority with modern style.",
    image: "/assets/IMG_1557.JPG",
    price: 3200000,
    currency: "NGN",
  },
  {
    id: "outfit-012",
    name: "Modern Business Classic",
    slug: "modern-business-classic",
    description: "A contemporary take on classic business attire for the style-conscious professional.",
    image: "/assets/IMG_1559.JPG",
    price: 2600000,
    currency: "NGN",
  },
  {
    id: "outfit-013",
    name: "Distinguished Professional",
    slug: "distinguished-professional",
    description: "A look that combines professionalism with distinctive personal style.",
    image: "/assets/IMG_1979.JPG",
    price: 2900000,
    currency: "NGN",
  },
  {
    id: "outfit-014",
    name: "Refined Executive",
    slug: "refined-executive",
    description: "Exemplary executive style that speaks volumes about your attention to detail.",
    image: "/assets/IMG_1982.JPG",
    price: 3100000,
    currency: "NGN",
  },
  {
    id: "outfit-015",
    name: "Classic Sophistication",
    slug: "classic-sophistication",
    description: "Timeless elegance that never goes out of style, perfect for any formal occasion.",
    image: "/assets/IMG_4655.JPG",
    price: 3400000,
    currency: "NGN",
  },
  {
    id: "outfit-016",
    name: "Professional Elegance",
    slug: "professional-elegance",
    description: "A perfect balance of professional polish and sophisticated style.",
    image: "/assets/IMG_4656.JPG",
    price: 2700000,
    currency: "NGN",
  },
];
